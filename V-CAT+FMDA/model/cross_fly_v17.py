...
from .cross_fly_v2 import channel_crossmodel as crossmodel
...
class fly_1d(nn.Module):
    """feature fusion"""
    def __init__(self, kernel_size = 3, moduledim = 512, calculate_loss = False, dsl_init_sigma = 3.0, dsl_init_center = 0.0):
        super(fly_1d, self).__init__()

        self.Norm_2 = nn.BatchNorm1d(moduledim)
        self.Norm_3 = nn.BatchNorm1d(moduledim)

        self.cross = crossmodel(moduledim, kernel_size, dsl_init_sigma, dsl_init_center, calculate_loss)
        self.channel_cross = channel_crossmodel(moduledim, kernel_size, dsl_init_sigma, dsl_init_center, calculate_loss)

    def forward(self, value, query, query_all):

        # value_time = self.cross(value.permute(0, 2, 1), query).permute(0, 2, 1)
        value_time = self.cross(value, query_all)
