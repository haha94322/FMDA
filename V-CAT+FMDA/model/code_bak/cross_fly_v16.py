...
def pearson(x, y):
    ...
    """
    value_real, value_imag = torch.abs(rho), torch.angle(rho)

    rho = value_real# + torch.sigmoid(value_imag)
    rho = torch.tanh(rho)
    # print(rho.max())
    return rho
...
def loss_formulation(freqs, alpha = 1.0):
    ...
    loss = torch.exp(-(torch.abs(loss)) * 5)  #目的是为了使得0处损失最大，两端最小。  使得特征尽可能的不同。
...
class channel_crossmodel(torch.nn.Module):
    ...
    def _gaussian_blur(self, x):
        ...
        x = x.repeat(1, 1, 1)
        self.gauss_kernel = self._get_gaussian_kernel1d(self.kernel_size, self.sigmas, self.center_shift)
        self.gauss_kernel = self.gauss_kernel.repeat(x.shape[-2], x.shape[-2], 1).permute(1, 0, 2)
        feat = F.conv1d(x, self.gauss_kernel)
        return feat.view(bs, c, self.kernel_num, channel)
    ...
    def forward(self, x, q):
        ...
        x_shift = self._gaussian_blur(x)

        value_fft_shift = torch.fft.fft(x_shift, dim=-1)
        weight = pearson(value_fft_shift, query_fft_shift)  # ** 2
        ...
        if self.calculate_loss:
            loss = loss_formulation(x_shift.permute(2, 0, 1, 3))
            # print(loss)
