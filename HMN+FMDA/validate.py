...
if __name__ == '__main__':
    ...
    random.seed(0)
    for i in range(1):
        # seed = random.randint(1, 666)
        seed = 395
        ckpt = os.path.join(cfg.dataset.save_dir, 'ckpt', 'model_'+str(seed)+'.pt')
        ...
        test_loader_kwargs = {
            ...
            'motion_feat': cfg.dataset.motion_feat.format(cfg.dataset.name,'test'),
        ...
        if cfg.test.write_preds:
            ...
            if cfg.dataset.question_type in ['action', 'transition']: \
                ...
                instances = [
                    {'video_id': video_id, 'question_id': q_id, 'video_name': dict[str(q_id)][0],
            ...
            sys.stdout.write('~~~~~~ seed: {seed_num} ~~~~~~~\n'.format(
                seed_num=colored("{:.4f}".format(seed), "red", attrs=['bold'])))
            sys.stdout.write('~~~~~~ Test Accuracy: {test_acc} ~~~~~~~\n'.format(
                test_acc=colored("{:.4f}".format(acc), "red", attrs=['bold'])))
            sys.stdout.flush()
