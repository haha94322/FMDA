...
def train(cfg):
    ...
    optimizer_bak = deepcopy(optimizer)
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        for i, batch in enumerate(iter(train_loader)):
            ...
            if i == 0 and epoch == 0:
                ...
                flops, params = profile(model, inputs=(*batch_input, answers))
                print('FLOPs = ' + str(flops / 1000 ** 3) + 'G')
                print('Params = ' + str(params / 1000 ** 2) + 'M')

                with torch.no_grad():
                    macs = profile_macs(model, (*batch_input, answers))
            ...
            if cfg.dataset.question_type in ['action', 'transition']:
                ...
                # loss_object = criterion(logits[3], answers)
                # loss = 0.7 * loss +  0.1 * loss_app +  0.1 * loss_motion +  0.1 * loss_object
                loss = criterion(logits, answers)
                optimizer.zero_grad()
                loss.backward()
                total_loss += loss.detach()
        ...
        if cfg.dataset.question_type not in ['action', 'transition']:
            if (epoch + 1) % 5 == 0:
                optimizer = step_decay(cfg, optimizer, decay)
        else:
            if (epoch + 1) % 10 == 0:
                optimizer = step_decay(cfg, optimizer, decay)
        sys.stdout.flush()
        ...
        if pattrack == 4:
            optimizer, decay = step_restore(cfg, optimizer, optimizer_bak, decay)
...
def step_decay(cfg, optimizer, decay):
...
def step_restore(cfg, optimizer, optimizer_bak, decay):
    # compute the new learning rate based on decay rate
    logging.info("restore learning rate to {}".format(cfg.train.lr))
    sys.stdout.flush()
    for param_group, param_group_bak in zip(optimizer.param_groups, optimizer_bak.param_groups):
        param_group['lr'] = param_group_bak['lr']
...
def main():
    ...
    if not os.path.exists(log_file):
        os.mkdir(log_file)
    fileHandler = logging.FileHandler(os.path.join(log_file,
                                                   'stdout_{}.log'.format(time.strftime("%Y%m%d%H%M"))), 'w+')
    ...
    for i in range(1):
        ...
        cfg.seed = 42   #0.4232
        cfg.index = 2    #msvd  2,   frameqa 3    msrvtt   6
        ...
        ckpt_bert = os.path.join(cfg.dataset.save_dir, 'embedding', '%s_branch_embeddings_%s_%s_%s.h5' % (cfg.dataset.name, 'train', 395, cfg.index))
        # if not os.path.exists(ckpt_bert) or cfg.index <= 1:
        if not os.path.exists(ckpt_bert):
            continue
        train(cfg)
