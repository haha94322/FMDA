...
from model.cross_fly_v2 import fly_1d
...
class VGT(nn.Module):
    def __init__(
        ...
        self.config = BertConfig.from_pretrained(
            "/home/zhouhao/lmy/paper6/old/bert-base-uncased",
            num_hidden_layers=N,
            hidden_size=d_model,
    ...
    def get_vqa_embedding_rfcpos(self, video, language, language_lens):
        ...
        language_mean = language.view(bsize,-1, len_lan, dim_lan).mean(1)
        X_o = self.fly_1(X, language_mean.repeat(numc*numf, 1, 1))
    ...
    def forward(
        ...
        video_proj, language_mean = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len)

        video_proj = self.position_v(video_proj)
        attended_v = self.mmt(x=video_proj, attn_mask=video_mask)[0]
        # attended_v = self.fly_2(attended_v, language_mean) + attended_v
