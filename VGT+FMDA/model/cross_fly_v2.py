...
class crossmodel(torch.nn.Module):
    def __init__(self, moduledim, kernel_size, dsl_init_sigma, dsl_init_center, channels=None, e_lambda=1e-4):
        ...
        self.e_lambda = e_lambda
        self.Norm1 = nn.BatchNorm1d(moduledim)
        ...
        # nn.init.xavier_uniform_(self.sigmas)
        # nn.init.xavier_uniform_(self.center_shift)
    ...
    def forward(self, x, q):
        ...
        value_filter = value_fft_shift * weight# + (1 - weight) * query_fft_shift

        outputs = torch.abs(torch.fft.ifft(value_filter, n=x.size(-2), dim=-2))# + x
...
class channel_crossmodel(torch.nn.Module):
    ...
    def forward(self, x, q):
        ...
        outputs = torch.abs(torch.fft.ifft(value_filter, n=x.size(-2), dim=-2)) + x.unsqueeze(1)   #只使用这里的残差  39。4
...
class fly_1d(nn.Module):
    ...
    def __init__(self, kernel_size = 3, moduledim = 512, dsl_init_sigma = 3.0, dsl_init_center = 0.0):
        ...
        self.channel_cross = channel_crossmodel(moduledim, kernel_size, dsl_init_sigma, dsl_init_center)
        self.cross = crossmodel(moduledim, kernel_size, dsl_init_sigma, dsl_init_center)
        self.cross2 = crossmodel(moduledim, kernel_size, dsl_init_sigma, dsl_init_center)
    ...
    def forward(self, value, query):
        ...
        # print(value_channel.max(), value_channel.min(), value_channel.mean())
        # a = len(query.shape)
        # if len(query.shape) == 3:
        #     query = self.cross2(query, value)#.sum(1)

        # value_channel = self.channel_cross(value, query)# + value
        # print(value_channel.max(), value_channel.min(), value_channel.mean())
