...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        total_acc, count = 0, 0
        batch_mse_sum = 0.0
        total_loss, total_contrastive_loss, avg_loss = 0.0, 0.0, 0.0
        avg_loss = 0
        train_accuracy = 0
        for i, batch in enumerate(iter(train_loader)):
            ...
            if i == 0 and epoch == 0:
                ...
                logging.info('FLOPs = ' + str(macs / 1000 ** 3) + 'G')

            logits, loss_contrastive = model(*batch_input, answers)
            if cfg.dataset.question_type in ['action', 'transition']:
                ...
                loss.backward()
                total_loss += loss.detach()
                total_contrastive_loss += loss_contrastive.detach()
                avg_loss = total_loss / (i + 1)
                avg_contrastive_loss = total_contrastive_loss / (i + 1)
                nn.utils.clip_grad_norm_(model.parameters(), max_norm=12)
                optimizer.step()
            ...
            if cfg.dataset.question_type == 'count':
                ...
                sys.stdout.write(
                    "\rProgress = {progress}   ce_loss = {ce_loss}   avg_loss = {avg_loss}  cont_avg = {avg_cont_loss} train_acc = {train_acc}    avg_acc = {avg_acc}    exp: {exp_name}".format(
                        progress=colored("{:.3f}".format(progress), "green", attrs=['bold']),
                        ce_loss=colored("{:.4f}".format(loss.item()), "blue", attrs=['bold']),
                        avg_loss=colored("{:.4f}".format(avg_loss), "red", attrs=['bold']),
                        avg_cont_loss=colored("{:.4f}".format(avg_contrastive_loss), "red", attrs=['bold']),
        ...
        if cfg.val.flag:
            ...
            valid_acc, valid_loss = validate(cfg, model, val_loader, device, write_preds=False)
            acc_bak.append(valid_acc)
            loss_bak.append(valid_loss)
        ...
        if pattrack >= patience:
            break
    logging.info('Valid Accuracy:{}'.format(acc_bak))
    logging.info('Valid loss:{}'.format(loss_bak))
