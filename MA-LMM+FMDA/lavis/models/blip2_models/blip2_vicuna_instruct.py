...
from .cross_fly_v1 import fly_1d
...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        bnb_config = {
            "load_in_4bit": True,
            "bnb_4bit_quant_type": "nf4",  # 使用 NF4（NormalFloat4）量化
        ...
        self.llm_model = LlamaForCausalLM.from_pretrained(
            llm_model, quantization_config=bnb_config, torch_dtype=torch.float16
        )
    ...
    def forward(self, samples):
        ...
        question_llm = query_output.last_hidden_state[:, query_tokens.size(1):, :]
        bert_output = query_output.last_hidden_state[:, :query_tokens.size(1), :]
        bert_output = self.att_fly(bert_output, question_llm) + bert_output
        inputs_llm = self.llm_proj(bert_output)
    ...
    def predict_answers(
        ...
        **kwargs
        ...
        if prompt:
            if prompt.count("{}") == 3:
                text_input = [prompt.format(caption, question, options) for caption, question, options in
                              zip(samples["caption"], samples["text_input"], samples['option'], )]
            elif  prompt.count("{}") == 2:
                text_input = [prompt.format(caption, question) for caption, question in zip(samples["caption"], samples['text_input'], )]
            else:
                text_input = [prompt.format(question) for question in samples["text_input"]]
        else:
            text_input = samples["text_input"]

        text_input = [question for question in samples["text_input"]]
