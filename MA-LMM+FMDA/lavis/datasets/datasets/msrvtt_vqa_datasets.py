...
class MSRVTTVQADataset(VideoQADataset):
    ...
    def __getitem__(self, index):
        ...
        caption = self.subtitles[int(ann_video.split("video")[1])]
        question_org = self.text_processor(ann["question"])
        if len(self.prompt) > 0:
            question = self.prompt.format(question_org)
            question_context = self.prompt_context.format(caption, question_org)
