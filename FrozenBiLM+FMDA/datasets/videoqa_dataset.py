...
class VideoQA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # video_id_sub = self.data["key"].values[idx]
        ...
        if self.subs is not None and video_id in self.subs:
            sub = str_text.join(self.subs[video_id].split(" ")[:20])
        sub_bool = bool(sub)
        if not self.use_context or self.train:
            sub = ""
...
def build_videoqa_dataset(dataset_name, split, args, tokenizer):
    if dataset_name == "msvd":
        ...
        subtitles_path = args.tgif_subtitles_path
