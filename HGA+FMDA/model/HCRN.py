...
from .EncoderRNN import EncoderQns, EncoderVidHGA
from model import HGA
...
class InputUnitVisual(nn.Module):
    def __init__(self, k_max_frame_level, k_max_clip_level, spl_resolution, vision_dim, k_num, module_dim=512,
                 calculate_loss = False):
        super(InputUnitVisual, self).__init__()

        k_size = k_num

        self.clip_level_motion_cond = CRN(k_size, module_dim, calculate_loss)
        self.clip_level_question_cond = CRN(k_size, module_dim, calculate_loss)
        self.video_level_motion_cond = CRN(k_size, module_dim, calculate_loss)
        self.video_level_question_cond = CRN(k_size, module_dim, calculate_loss)
...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, module_dim, word_dim, k_max_frame_level,
                 k_max_clip_level, spl_resolution, vocab, question_type, k_nums, calculate_loss):
        ...
        if self.question_type in ['action', 'transition']:
            ...
            self.mot_proj = nn.Linear(192, 2048)
            # vid_encoder = EncoderVidHGA(vision_dim * 2, module_dim, input_dropout_p=0.3,
            #                                          bidirectional=False, rnn_cell='gru')

            # self.HGA = HGA.HGA(vid_encoder)
            self.HGA = LSTMCrossCycleGCNDropout(
                2048,
                2048,
                1,
                1,
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, question,
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            video_appearance_feat, video_motion_feat = video_appearance_feat.flatten(-2).mean(-1), video_motion_feat.flatten(-2).mean(-1).permute(0, 2, 1)
            video_motion_feat = self.mot_proj(video_motion_feat)

            video = torch.cat((video_appearance_feat, video_motion_feat),dim=-1)

            visual_embedding = self.HGA(video, question_embedding, text_embedding)
