...
from ..EncoderRNN import EncoderVidHGA
...
class LSTMCrossCycleGCNDropout(nn.Module):
    ...
    def __init__(
        ...
        self.video_encoder = EncoderVidHGA(4096, 512, input_dropout_p=0.3, bidirectional=False, rnn_cell='gru')
        ...
        if answer_vocab_size is not None:
            ...
            self.fusion = fusions.Block([hidden_size, hidden_size], 512)
            self.fc_fusion = nn.Linear(hidden_size, 512)
    ...
    def model_block(
            self, video, sentence_inputs,
            question_length):

        ### question encoder
        q_output, s_hidden = question_length, sentence_inputs
